...
export default function Pay() {
    ...
    const [isSplitType, setIsSplitType] = useState(false);
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (orderId) {
                    ...
                    setIsSplitType(orderRes.data.split_type);
                }
            } catch (error) {
    ...
    const handleSubmit = async () => {
        // 각자먹기일 때만 메뉴 선택 필수
        if (!isSplitType && selectedMenus.length === 0) {
            alert('최소 1개 이상의 메뉴를 선택해주세요.');
            return;
        }

        try {
            // 1. 각자먹기일 때만 장바구니에 메뉴 추가
            if (!isSplitType) {
